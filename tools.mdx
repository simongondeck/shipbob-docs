...
```mermaid
graph TD
    A[Frontend] -->|Collect payment info & confirm intent| B[Stripe API]
    B:::stripe -->|Payment is attempted| C[Payment succeeds]
    C -->|Update subscription's payment method| D[Backend]
    D -->|Display payment confirmation| A

    classDef stripe fill:#ADD8E6,stroke:#000,stroke-width:2px;
```

```mermaid
%%{init: {'themeVariables': {'actorBkg': '#ADD8E6', 'actorTextColor': '#000'}}}%%
sequenceDiagram
    participant Frontend
    participant Backend
    participant StripeAPI as Stripe API
